...
# -*- coding: utf-8 -*-
"""
Script corrigé pour régénérer les fichiers HTML avec les données complètes.
"""
...
def create_enhanced_html(json_data, specialty_name):
    """Crée un HTML enrichi avec les données complètes et les fonctionnalités corrigées."""
    
    # Extraire les statistiques
    total_sections = len(json_data.get('sections', []))
    total_pathologies = 0
    
    # Compter les pathologies (gestion des deux formats : 'data' ou 'pathologies')
...
'''
    ...
    for i, section in enumerate(json_data.get('sections', []), 1):
        ...
        section_name = section_name.replace(f'{i}. ', '')
...
'''.format(total_pathologies, total_pathologies)
    ...
    for section_idx, section in enumerate(json_data.get('sections', []), 1):
        ...
        # Nettoyer le nom de section
        section_name = section_name.replace(f'{section_idx}. ', '')
        
        # Obtenir les pathologies (gestion des deux formats)
...
'''
        ...
        for pathology in pathologies:
            ...
            if isinstance(pathology, dict):
                ...
                anamnese = format_field_with_highlights(pathology.get('Anamnèse', pathology.get('anamnese', '')))
                examen = format_field_with_highlights(pathology.get('Examen Physique', pathology.get('examen_physique', '')))
                procedures = format_field_with_highlights(pathology.get('Procédures/Examens', pathology.get('procedures_examens', '')))
                therapie = format_field_with_highlights(pathology.get('Thérapie', pathology.get('therapie', '')))
                commentaires = format_field_with_highlights(pathology.get('Commentaires', pathology.get('commentaires', '')))
            else:
                continue
            ...
            if 'urgent' in content_str or 'urgence' in content_str:
                data_tags.append('urgent')
            if 'warning' in content_str or 'attention' in content_str or 'alerte' in content_str:
                data_tags.append('warning')
            if 'dosage' in content_str or 'mg' in content_str or 'ml' in content_str or 'ui' in content_str:
                data_tags.append('dosage')
            if 'osce' in content_str or 'ecos' in content_str or 'tip' in commentaires.lower() or 'conseil' in commentaires.lower():
...
def format_field_with_highlights(text):
    ...
    text = re.sub(r'\*\*([0-9]+\s*ml[^*]*)\*\*', r'<span class="dosage">\1</span>', text)
...
if __name__ == "__main__":
    print("🔧 Régénération des fichiers HTML avec les données complètes...")
    print("-" * 50)
    process_files()
    print("-" * 50)
